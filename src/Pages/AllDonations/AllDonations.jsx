  ...
  {donations.length === 0 ? (
    ...
    <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {donations.map((donation) => (
        <div
          key={donation._id}
          className="bg-base-100 rounded-2xl shadow-lg border border-primary flex flex-col overflow-hidden hover:shadow-2xl transition-all h-full"
        >
          ...
          <div className="p-6 flex flex-col flex-grow space-y-2">
            ...
            <Link
              ...
              className="mt-auto block w-full text-center py-2 px-4 rounded-lg border border-primary text-primary hover:bg-primary hover:text-white transition"
            >
              View Details
            </Link>
          </div>
